...
import sample.math.Gauss;
import sample.math.Matrix;
import sample.math.Solve;

public class ReadDataReal {
    // Class variables
    private String data;
    ...
    public ReadDataReal() {
        ...
        try {
            ...
            if (NumberOfUnknowns > 1) {
                ...
                double[][] dataNumbers = EditData.toDoubleArray(dataMatrix);
                dataNumbers = Gauss.Gausseli(dataNumbers);
                Controller.Output += "Final matrix after Gauss:\n";
                Controller.Output += Matrix.MatrixToString(dataNumbers);
                Solve.SolveReal(dataNumbers);
                Controller.output.setText(Controller.Output);
            } else {
                throw new Exception();
            }
        } catch (Exception A) {
            Controller.output.setText("Wrong input!(32)");
        }
